...
SkillVerify connects LinkedIn, ingests evidence (resume, GitHub, certificates), runs AI-generated adaptive assessments, and produces an **explainable** 0–100 confidence score per skill — never a black-box number. Recruiters get a dashboard to search and filter candidates by verified skill instead of skimming resumes.
...
- **A compounding moat, not just a feature.** Recruiter-confirmed hiring outcomes feed back into the scoring model, so the weighting gets more accurate the longer the product runs — something a competitor can't copy by cloning the UI.
...
Concept + prototype + full PRD complete. Pre-build stage — this is a portfolio/case-study artifact, not a production product.

## Author

Built by Ella, NUST SEECS.
